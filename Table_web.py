...
class TableImage:
    ...
    def draw(self):
        ...
        for (r, c), text in self.cells.items():
            ...
            line_heights = [draw.textbbox((0, 0), line, font=font)[3] - draw.textbbox((0, 0), line, font=font)[1] for line in lines] or [self.font_size]
...
# Allow override of font paths (optional)
win_font = r"C:\Windows\Fonts\VERDANA.TTF"
win_bold = r"C:\Windows\Fonts\VERDANAB.TTF"

# Check for existence and warn if missing
font_path_to_use = win_font
bold_path_to_use = win_bold
if not os.path.exists(win_font) or not os.path.exists(win_bold):
    st.warning(f"Windows font files not found at {win_font} / {win_bold}. Falling back to local 'verdana.ttf' and 'verdanab.ttf' (or system default).")
    # Use local filenames as fallback (you can bundle these with the app)
    font_path_to_use = "verdana.ttf"
    bold_path_to_use = "verdanab.ttf"
...
# Note: pass the font paths we've resolved above
table = TableImage(
    rows=rows,
    cols=cols,
    ...
    bold_font_path=bold_path_to_use,
    font_size=11,
    line_width=1,
    wrap_width=29
)

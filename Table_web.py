...
# --- TableImage class (fixed __init__ and save uses PNG by default) ---
class TableImage:
    ...
    def save(self, filename):
        img = self.draw()
        # default to PNG to avoid JPEG compression artifacts
...
# if using Type 1, ensure col_width is defined
if table_type.startswith("Type 1"):
...
scale_to_column = st.checkbox("Scale preview to column width (may resample)", value=False)

# Generate and download image
if st.button("Generate Table Image"):
    img = table.draw()
    # show preview (PNG avoids compression halo)
    st.image(img, caption="Generated Table", use_column_width=scale_to_column)

    # Convert to bytes for download using PNG (lossless)
    buf = io.BytesIO()
    img.save(buf, format="PNG", optimize=True)
    byte_im = buf.getvalue()
    st.download_button(
        label="Download Table Image (PNG)",
        data=byte_im,
        file_name="table.png",
        mime="image/png"
    )
